...
/// @title NotarizationProxyV3Test
/// @author Anto Benedetti (anto.benedetti@cea.fr)
/// @notice This contract tests all the functions in the NotarizationProxyV3
/// contract
contract NotarizationProxyV3Test is Test, Utils {
    ...
    NotarizationProxyV3 public castedProxyV3;

    /// @notice Sets up the test environment
    /// @dev Deploys proxy & logicV3 contracts, links them, casts the proxy contract to the tested contract before each
    /// test
    function setUp() public {
        ...
        proxy = new ERC1967Proxy(address(logicV3), "");

        // Cast the proxy contract to the tested contract for more convenient testing
        castedProxyV3 = NotarizationProxyV3(address(proxy));
        castedProxyV3.initialize();
    }
    ...
    function test01_addFile_variantLengths() public {
        for (uint256 i = 1; i <= 100; i++) {
            ...
            vm.expectEmit(true, true, true, true, address(castedProxyV3));
            emit FileNotarized(address(this), fileName, fileHash);
            castedProxyV3.addFile(fileName, fileHash);

            // Check if file name & file hash are stored in the contract
            string memory fileNameFromContract = castedProxyV3
                .getFilename(fileHash);
            NotarizationProxyV3.File memory fileDetailsFromContract = castedProxyV3
    ...
    function test03_updateFile() public {
        ...
        for (uint256 i = 1; i <= 100; i++) {
            ...
            castedProxyV3.updateFile(fileName, newFileHash);

            bytes32 fileHashFromContract = castedProxyV3.getFileHash(
                fileName
            );
            assertEq(
    ...
    function test06_deleteFile() public {
        for (uint256 i = 1; i <= 100; i++) {
            ...
            castedProxyV3.deleteFile(fileName);
    ...
    function test07_getFileName() public {
        ...
        string memory fileNameFromContract = castedProxyV3.getFilename(
            fileHash
        );
        assertFalse(
    ...
    function test08_getFileDetails() public {
        string memory fileName = randomString(10);
        bytes32 fileHash = keccak256(abi.encode(fileName));
        NotarizationProxyV3.File memory wrongFileDetails = NotarizationProxyV3.File(
    ...
    function test10_getFileOwner() public {
        for (uint256 i = 1; i <= 100; i++) {
            ...
            address fileOwnerFromContract = castedProxyV3.getFileOwner(
                fileName
            );
            assertFalse(
    ...
    function test11_getFileCreatedAt() public {
        for (uint256 i = 1; i <= 100; i++) {
            ...
            uint256 fileCreatedAtFromContract = castedProxyV3
    ...
    function test12_getFileLastModifiedAt() public {
        for (uint256 i = 1; i <= 100; i++) {
            ...
            uint256 fileLastModifiedAtFromContract = castedProxyV3
