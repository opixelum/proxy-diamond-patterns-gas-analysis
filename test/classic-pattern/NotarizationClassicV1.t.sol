...
import {NotarizationClassicV1} from "../../src/classic-pattern/NotarizationClassicV1.sol";
import {Test} from "forge-std/Test.sol";
import {Utils} from "../../script/Utils.s.sol";

/// @title NotarizationClassicV1Test
/// @author Anto Benedetti (anto.benedetti@cea.fr)
/// @notice This contract tests all the functions in the NotarizationClassicV1
/// contract
/// @dev Tests can only be run using Forge
contract NotarizationClassicV1Test is Test, Utils {
    // Define events emitted by the tested contract for testing
    event FileNotarized(string indexed name, bytes32 indexed hash);

    NotarizationClassicV1 public notarizationClassicV1;

    /// @notice Sets up the test environment
    /// @dev Creates a new instance of the tested contract before each test
    function setUp() public {
        notarizationClassicV1 = new NotarizationClassicV1();
    }
    ...
    function test1_AddFile_variantLengths() public {
        for (uint256 i = 1; i <= 100; i++) {
            ...
            vm.expectEmit(true, true, true, true, address(notarizationClassicV1));
            emit FileNotarized(fileName, fileHash);
            notarizationClassicV1.addFile(fileName, fileHash);

            // Check if file name & file hash are stored in the contract
            bytes32 fileHashFromContract = notarizationClassicV1.getFileHash(fileName);
            string memory fileNameFromContract = notarizationClassicV1.getFileName(fileHash);
    ...
    function test6_compareHashes() public {
        for (uint8 i = 1; i <= 100; i++) {
            // Use different lengths to avoid collisions
            bytes32 fileHash = keccak256(abi.encode(randomString(i)));
            bytes32 wrongFileHash = keccak256(abi.encode(randomString(i + 1)));

            assertTrue(
                notarizationClassicV1.compareHashes(fileHash, fileHash), "Function returns false when comparing equal hashes"
            );
            assertFalse(
                notarizationClassicV1.compareHashes(fileHash, wrongFileHash),
