...
/// hashes.
/// @dev This contract uses the proxy pattern with the UUPS upgradeability pattern. Ensure proper access control and file existence
/// checks before performing sensitive operations.
contract NotarizationProxyV3 is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function updateFile(
        ...
        bytes32 newHash
    ) external fileExists(filename) onlyFileOwner(filename) {
